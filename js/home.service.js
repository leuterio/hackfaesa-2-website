...
'use strict';
    ...
    function HomeService($http, $log, $q) {
        ...
        this.obterLatLong = obterLatLong;
        ...
        function Logar(user){
            user.nome = "José da Silva";
            return user;
        }
        function obterLatLong(endereco){
            var deferred = $q.defer();
            var url = "https://maps.googleapis.com/maps/api/geocode/json?address="+endereco.split(" ").join("+")+"&key=AIzaSyBtDbeEPMIUBPWCHiXwmOSgDjLAFKkpZv4";
            console.log("url", url);
            $http.get(url)
